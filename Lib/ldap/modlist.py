...
def addModlist(entry,ignore_attr_types=None):
  ...
  ignore_attr_types = list_dict(list(map(str.lower,(ignore_attr_types or []))))
  modlist = []
...
def modifyModlist(
  ...
  """
  ignore_attr_types = list_dict(list(map(str.lower,(ignore_attr_types or []))))
  case_ignore_attr_types = list_dict(list(map(str.lower,(case_ignore_attr_types or []))))
  modlist = []
  attrtype_lower_map = {}
  ...
  for attrtype in list(new_entry.keys()):
    ...
    if not old_value and new_value:
      ...
      if not replace_attr_value:
        ...
        delete_values = []
        for v in old_value:
          if not v in new_value_dict:
            replace_attr_value = 1
            break
        add_values = []
        if not replace_attr_value:
          for v in new_value:
            if not v in old_value_dict:
              replace_attr_value = 1
              break
