...
def By2Str(datain):
    """ Traverse over  ldapsearch result structure and convert all byte strings (bytes) to string (str).
        Dict keys are untouched.
        Return value is complete new structure contains only decoded string.
    """
    dataout = datain.__new__(type(datain))
    if(type(datain) is dict):
        for key in datain:
            dataout[key] = By2Str(datain[key])
    elif(type(datain) is list):
        for value in datain:
            dataout.append(By2Str(value))
    elif(type(datain) is tuple):
        datalist = []
        for value in datain:
            datalist.append(By2Str(value))
        dataout = tuple(datalist)
    elif(type(datain) is bytes):
...
def str2dn(dn,flags=0):
  ...
  if not dn:
    return []
  return By2Str(ldap.functions._ldap_function_call(None,_ldap.str2dn,dn,flags))
